...
const STATUS_COLOR: Record<SavedApplication['status'], string> = {
  ...
  'draft-ready': 'text-[#C9A84C] bg-[#C9A84C]/10 border-[#C9A84C]/20',
                ...
                >
                  <div className="flex items-start justify-between gap-4">
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-1 text-sm text-neutral-400">{app.artistName}</p>
                      <div className="mt-2 flex items-center gap-4 text-xs text-neutral-600">
                        ...
                        {app.session.conversation.length > 0 && (
                          <span>
                            {Math.ceil(app.session.conversation.filter((m) => m.role === 'assistant').length)} Q&amp;A
                          </span>
                        )}
                      </div>
                    </div>
                          ...
                          <div key={s} className="flex flex-1 flex-col items-center gap-1">
                            <div
                              className={[
                                ...
                                stageReached ? 'bg-[#C9A84C]' : 'bg-neutral-800',
                              ].join(' ')}
                            />
                            <span className={[
                              'text-[10px]',
                              stageReached ? 'text-[#C9A84C]' : 'text-neutral-700',
                            ].join(' ')}>
                              {labels[i]}
                            </span>
                          </div>
